...
#define Init_Variables std::string where = ""; std::string strBreak = ""; int switchVal = 0; int ForSwitch  = 0; std::string label1 = ""; std::string label2 = ""; int loopNum = 0;
//a is a label. Declare variables before calling this. This function must be called before using any gotos.
#define Start(a) for(;;) {for(int a = 0; a < 1; a++){ if(where == "") {
//This function must be called at the end of main.
#define End where = ""; break; }}if (where == "") {break;} }
//This creates a label called x.
#define Define_Pos(x) where = #x;} if(where==#x) {
//This goes to a label called x.
#define Goto(x) where = #x; break;
//x is a string not a var.
#define Define_Pos_String(x) where = x;} if(where==x) {
//x is a string not a var.
